import { Patch, PatchObservable, PatchObservableLike, PatchObservableOperator, Unit, UnitRange } from "../patch-observable";

export function mapUnits<T, U>(map: (value: T) => U): PatchObservableOperator<T, PatchObservableLike<U>> {
	return (source: PatchObservableLike<T>) => {
		let resolved = false;
